...
| `reclaimDisabledProxies` | boolean | `true` | 代理槽不足时，自动回收**已禁用**凭据占用的代理槽给新号 |

**代理槽不足时自动腾位**：启用代理池后，如果空闲槽不够新号用，会自动回收**已禁用**凭据占着的代理槽
（被回收的凭据保持禁用，只是不再占槽；重新启用时需要再分配代理）。
回收顺序按「死透程度」：封号 / 认证失败 / 额度耗尽 / 配置无效 / 余额不足最先，
失败计数类次之，管理员手动禁用的最后；同档内最久没用过的先回收。
**启用中的凭据一律不动**；绑在已禁用、快过期或处于失败冷却的代理上的槽也会跳过（解了也分配不出去）。

**存活时长**：上号记录会显示每个号从上号到废掉活了多久。号还在用就持续计时；
被删除或因封号 / 认证失败 / 额度耗尽 / 余额不足 / 配置无效等**不可恢复**原因禁用即算废，时长就此定格。
废掉的时刻取自 `error_logs` 里持久化的自动禁用事件（比事后扫描发现的时间准，服务重启也不丢）。
手动禁用、连续失败这类还能恢复的不算废，继续计时。
...
**上号记录**（每个成功上号的 Key 一条：凭据 ID、脱敏 Key、订单号、所绑代理、**存活时长**、状态、来源）、
