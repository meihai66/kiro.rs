...
- **代理测试把 407 代理误判为可用** — 去掉明文 HTTP 兜底：不少代理对明文转发放行、对 CONNECT 返回 407，明文成功会把实际完全不可用的代理判为可用（真实流量全是 HTTPS）；判定目标改为 Kiro 认证上游（经 HTTPS 隧道收到非 407 响应才算可用），ipify 仅用于尽力获取出口 IP；不再缓存复用测试 Client（复用旧隧道不会重新走代理认证，代理失效后仍测出可用）；错误信息展开完整错误链，407 给出可操作提示 (`src/kiro/proxy_pool.rs`)
- **凭据导入网络错误丢失根因** — 错误信息用 `{:#}` 带出 reqwest 根因（连接被拒/代理认证/DNS/TLS）；"error sending request" 归类为上游服务错误而非内部错误 (`src/admin/service.rs`)
...
- **请求体大小上限支持按模型配置覆盖** — 新增 `perModelBodyLimits` 规则（`match`/`matchType`/`maxBytes`，匹配规则同 `model_mapping`，自上而下第一条命中）。请求预检改为按模型解析上限，未命中回退全局 `maxRequestBodyBytes`；Admin GET/PUT 读写并持久化，设置页「输入压缩」卡片新增可视化规则编辑器
- **凭据「最近 1000 次」色条改为按失败率绿→黄→红渐变** — 失败与 429 合并计入失败率，每格按失败率在绿→黄→红连续取色（走 hue 过渡避免脏褐色）；全成功为纯绿，采用容忍度曲线让少量失败仍偏绿、接近全失败才转红，解决旧线性混色下偶发失败「看不出区别」的问题

### 修复

- **图片声明格式与实际字节不符时按内容嗅探自动修正** — 上游偶发 `IMAGE_MIME_MISMATCH`（如 PNG 字节被声明为 `image/jpeg`）。`process_image` 现以 `with_guessed_format` 嗅探到的真实格式为准打标签：小图直通改用真实格式，重编码按真实格式编码，嗅探失败/不支持则回退声明格式并强制重编码，保证字节内容与 `format` 标签一致
